...
# function takes in an array of tuples and determines the length of a path through all the points in the order which they are provided
# I: <list> of <tuples>
# O: <float> distance
def total_distance(points):
    return sum([distance(point, points[index + 1]) for index, point in enumerate(points[:-1])])

# function finds the shortest route to visit all the cities by bruteforce. never use on long lists!
# I: <list> of <tuples>
# O: <float> distance
...
# function uses travelling salesman via going to the nearest city. Faster version, typically returns a sollution around 25% longer than brute force method
# I: <list> of <tuples>
# O: <float> distance
...
# I: takes in vectors x, y <list> or <tuple>
# O: outputs <list> of the vector sum
def vector_sum(x, y):
    if len(x) != len(y):
        return None
...
# I: reads in filename <str>
# O: outputs <dict> containing data sorted by category
...
# I: <dictionary> file containing the categories
# O: <list> of data
...
# I: 3 points  (tuples or list) in 2D R space
# O: <float> distance
...
class Kmeans():
    def __init__(self, args, k, data,user_centroids):
        ...
        self.centroids = self.pick_centroids()
    ...
    def pick_centroids(self):
        '''this function allows for evolution of centroid points over iterations of kmeans
        This function adjusts the number of centroids passed to the function to the optimal k'''
        if self.user_centroids:
            if len(self.user_centroids[0])!=self.numDimensions:
                return self.generate_random_centroids()
            else:
                if self.k < len(self.user_centroids):
                    while self.k < len(self.user_centroids):
                        del self.user_centroids[0]
                elif self.k > len(self.user_centroids):
                    while self.k > len(self.user_centroids):
                        temp = self.generate_random_centroids()
                        self.user_centroids.append(temp[0])
                return self.user_centroids
        else: 
            return self.generate_random_centroids()
...
def main(args):
    ...
    # Define History files (contain all of progression data)
    outputfile_centroids_history = open(str(output_filename + "_centroids"), "w")
    outputfile_points_hist = open(str(output_filename + "_points"), "w")
    outputfile_TraceMap_hist = open(str(output_filename + "_TraceMap"), "w")
    outputfile_area_hist = open(str(output_filename + "_area.csv"), "w")
    outputfile_path_hist = open(str(output_filename + "_path.csv"), "w")
    outputfile_pathdist_hist = open(str(output_filename + "_pathdist.csv"), "w")
    ...
    # set data parameters and validate dimensionality
    ...
    # Run kmeans on global set of data; no local scanning
    SSEs_global = []
    for x123 in range(len(data_input)):
        global_k = Kmeans(options, x123 + 1, data_input,None)
        global_k.run()
        SSEs_global.append([x123 + 1, global_k.SSE()])
    sse_distances = find_elbow(SSEs_global)
    best_global_k = sse_distances.index(max(sse_distances))

    global_run = Kmeans(options, best_global_k + 1, data_input, None)
    global_run.run()
    global_centroids = open(str(output_filename + "_global_centroids.csv"), "w")
    global_points = open(str(output_filename + "_global_points.csv"), "w")

    for gcentroids, item in enumerate(global_run.centroids):
        global_centroids.write(str(item)[1:len(str(item)) - 1] + "\n")
    for gpoints, item in enumerate(data_input):
        global_points.write(str(item)[1:len(str(item)) - 1] + "\n")
    global_centroids.close()
    global_points.close()


    # generate array backbones for troublshooting historical data
    ...
    # Main window scanning loop
    # for each window
    for x in range(numScans):
        # Write window number
        outputfile_centroids_history.write("> Window " + str(x + 1) + " Clusters >>>>>>>>>>>>>>>>\n")
        outputfile_points_hist.write("> Window " + str(x + 1) + " Points >>>>>>>>>>>>>>>>\n")
        ...
        for x2 in range(window_size):
            k_size_means = Kmeans(options, x2 + 1, window_data,None)
            k_size_means.run()
        ...
        k_means_run = Kmeans(options, optimal_k + 1, window_data, c_input)
        c_input = k_means_run.centroids
        k_means_run.run()

        # if data is in 2D, sort centroids in CCW order, otherwise don't do anything
        if toodee:
            ...
            # calculate the area of the centroid gemoetry (given that it is 2D)
            area = shoelace(formatted_ordered_ctd_pts)
            outputfile_area_hist.write(str(area)+"\n")
        else:
            formatted_ordered_ctd_pts = k_means_run.centroids

        # Regardless of dimentionality, determine minmum path distance between the points and save to file (with path)
        pathdist = travelling_salesman(tuple(formatted_ordered_ctd_pts))
        outputfile_path_hist.write(str(pathdist)+"\n")
        outputfile_pathdist_hist.write(str(total_distance(pathdist))+"\n")


        # write tracemap data to global history file
        outputfile_TraceMap_hist.write("> Window " + str(x + 1) + " Tracemap >>>>>>>>>>>>>>>>\n")
        for x4, item in enumerate(k_means_run.mega_dump()):
            outputfile_TraceMap_hist.write(str(item)[1:len(str(item)) - 1] + "\n")

        # write centroid points to global and local data files
        outputfile_centroids_local = open(str(output_filename+"_c"+str(x+1)+".csv"),"w")
        for x8, item in enumerate(formatted_ordered_ctd_pts):
            outputfile_centroids_local.write(str(item)[1:len(str(item)) - 1] + "\n")
            outputfile_centroids_history.write(str(item)[1:len(str(item)) - 1] + "\n")
        outputfile_centroids_local.close()

        # write data points to global and local history data files
        outputfile_points_local = open(str(output_filename+"_w"+str(x+1)+".csv"),"w")
        for x3, item in enumerate(window_data):
            outputfile_points_hist.write(str(item)[1:len(str(item)) - 1] + "\n")
            outputfile_points_local.write(str(item)[1:len(str(item)) - 1] + "\n")
        outputfile_points_local.close()

    # Write troubleshooting data to csv files.
    ...
    # close data files
    outputfile_centroids_history.close()
    outputfile_points_hist.close()
    outputfile_TraceMap_hist.close()
    outputfile_area_hist.close()
    outputfile_path_hist.close()
    outputfile_pathdist_hist.close()
